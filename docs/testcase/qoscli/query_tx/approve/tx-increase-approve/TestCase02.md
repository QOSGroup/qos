...
-- `--coins`没有单位
```
$ qoscli tx increase-approve --coins 10000 --from test --to test01
```
-- `--from`没有`--coins`指定的QSC
```
$ qoscli tx increase-approve --coins 10000Star --from test --to test01
```
# Output
-- `--coins`没有单位
```
...
-- `--from`没有`--coins`指定的QSC
```
$ qoscli tx increase-approve --coins 10000Star --from test --to test01 --indent
